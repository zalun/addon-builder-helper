...
function onContentScriptRequest(addonManager, args, callback) {
  try {
    // For now, `send` accept only one argument: `data` object
    let data = args[0];
    if (!('cmd' in data && typeof(data.cmd) == 'string'))
      return callback({success: false, msg: "bad request"});
    switch (data.cmd) {
    case "isInstalled":
      callback({
        success: true,
    ...
    case "install":
      ...
      addonManager.install(data.contents, function (success) {
        callback({
          success: success,
      ...
      break;
    case "toggleConsole":
      let message = toggleConsoleWindow();
      callback({
        success: true,
        msg: message
      });
